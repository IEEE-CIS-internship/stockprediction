...
def main():
    ...
    for h in horizons:
        ...
        for ticker in tickers:
            ...
            moe_model.load_state_dict(torch.load(os.path.join(models_dir, f"{ticker}_moe_h{h}.pt")))
            moe_model.eval()
            with torch.no_grad():
    ...
    df_overall.to_csv(os.path.join(tab_dir, "overall_results.csv"), index=False)
    with open(os.path.join(tab_dir, "overall_results.md"), "w", encoding="utf-8") as f:
    ...
    df_regimes.to_csv(os.path.join(tab_dir, "per_regime_results.csv"), index=False)
    with open(os.path.join(tab_dir, "per_regime_results.md"), "w", encoding="utf-8") as f:
    ...
    df_transition.to_csv(os.path.join(tab_dir, "transition_period_results.csv"), index=False)
    with open(os.path.join(tab_dir, "transition_period_results.md"), "w", encoding="utf-8") as f:
    ...
    df_per_stock.to_csv(os.path.join(tab_dir, "per_stock_breakdown.csv"), index=False)
    with open(os.path.join(tab_dir, "per_stock_breakdown.md"), "w", encoding="utf-8") as f:
    ...
    df_backtest.to_csv(os.path.join(tab_dir, "backtest_results.csv"), index=False)
    with open(os.path.join(tab_dir, "backtest_results.md"), "w", encoding="utf-8") as f:
    ...
    for h in horizons:
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(fig_dir, f"overall_accuracy_h{h}.png"), dpi=150)
        plt.close(fig)
